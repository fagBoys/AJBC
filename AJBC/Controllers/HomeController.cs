    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index(string Firstname, string Lastname, string Subject, string Emailaddress, string Message)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                MailboxAddress from = new MailboxAddress("AJ Building ", "ajbbuilding49@gmail.com");
                message.From.Add(from);

                MailboxAddress to = new MailboxAddress("AJ Building", "mjnsoftzerone@outlook.com");
                message.To.Add(to);
                ...
                MailboxAddress from2 = new MailboxAddress("AJ Building", "ajbbuilding49@gmail.com");
        ...
        [HttpPost]
        public async Task<IActionResult> Contact(ContactViewModel? contact, string? Firstname, string? Lastname, string? Emailaddress, string? Subject, string? Message)
        {
            ...
            else
            {
                if (!ModelState.IsValid)
                {
                    return View(!ModelState.IsValid ? contact : new ContactViewModel());
                }
                else
                {
                    context.Contact.Add(new Contact { Firstname = contact.Firstname, Lastname = contact.Lastname, Subject = contact.Subject, Email = contact.Email, Message = contact.Message });
                    context.SaveChanges();
